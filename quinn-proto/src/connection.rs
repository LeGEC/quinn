...
impl Connection {
    ...
    pub fn handle_decode(
        ...
        match partial_decode.finish(crypto.pn_decrypt_key()) {
            Ok((packet, rest)) => {
                self.handle_packet(mux, now, packet, new_crypto);
                rest
            }
            Err(e) => {
    ...
    fn handle_packet(
        ...
        let result = match self.decrypt_packet(was_handshake, &mut packet, crypto_update) {
            ...
            Err(None) => {
                ...
                if self.check_for_stateless_reset(mux, &packet) {
    ...
    fn check_for_stateless_reset(&mut self, mux: &mut impl Multiplexer, packet: &Packet) -> bool {
